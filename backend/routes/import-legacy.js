/**
 * import-legacy.js
 * Backward-compatible import routes for existing workspace/group views.
 * These endpoints accept the old JSON body format { csvText, eventId, ageGroupId }.
 *
 * The new event-scoped multipart routes are in import.js.
 * This file preserves the existing two-phase CSV preview/commit behavior
 * so GroupsView and WorkspacePage continue working during the transition.
 *
 * @deprecated Use /api/events/:eventId/import/upload instead.
 */
...
// Reuse normalizers from parse-upload since CSV is text here
...
const ALIASES = {
  first_name: ['first_name','firstname','first','player_first_name','given_name','participant_first_name'],
  last_name: ['last_name','lastname','last','player_last_name','surname','participant_last_name'],
  jersey_number: ['jersey_number','jersey','number','jersey_number_during_tryouts_','jersey_','tryout_number','uniform_number','player_number'],
  position: ['position','position_s_','positions','position_skater'],
  shot: ['shot','shoots','shot_hand','shooting_hand','shot_l_or_r','shot_l_or_r_','shot_left_or_right','preferred_shot','preferred_shot_hand','dominant_hand','stick_hand','handedness','hand'],
  ...
  date_of_birth: ['date_of_birth','dob','birthdate','birth_date','birthday','date_of_birth_mm_dd_yyyy','participant_dob'],
  gender: ['gender','sex','participant_gender'],
  external_id: ['external_id','profile_id','profileid','se_id','sports_engine_id','registration_id'],
};
...
function parseWillTryout(raw) {
  ...
  return !['no','false','0','n'].includes(String(raw).toLowerCase().trim());
}
...
router.get('/csv-template', ...guard, (_req, res) => {
  console.warn('[deprecated] GET /api/import/csv-template — use /api/events/:eventId/import/players-template');
  const csv = [
...
router.post('/preview', ...guard, async (req, res) => {
  console.warn('[deprecated] POST /api/import/preview — use /api/events/:eventId/import/upload');
  ...
  try {
    ...
    const [existingRes, externalRes, blocksRes] = await Promise.all([
      pool.query(`SELECT per.jersey_number FROM player_event_registrations per WHERE per.age_group_id = $1 AND per.event_id = $2`, [ageGroupId, eventId]),
      pool.query(`SELECT external_id FROM players WHERE external_id IS NOT NULL`),
      pool.query(`SELECT sb.split_method, s.id AS session_id, s.name AS session_name, s.start_time, s.last_name_start, s.last_name_end, s.jersey_min, s.jersey_max FROM session_blocks sb JOIN sessions s ON s.block_id = sb.id WHERE sb.age_group_id = $1 AND sb.event_id = $2 AND sb.block_type = 'skills' ORDER BY s.start_time`, [ageGroupId, eventId]),
    ]);
    const existingJerseys     = new Set(existingRes.rows.map(r => r.jersey_number));
    const existingExternalIds = new Set(externalRes.rows.map(r => r.external_id));
    const sessions = blocksRes.rows;
    for (const row of rows) {
      const fields = mapPlayerFields(row);
      const shotResult = normalizeShot(fields.shot);
      const result = {
        raw: row, firstName: fields.first_name || '', lastName: fields.last_name || '',
        jerseyNumber: parseInt(fields.jersey_number) || null, position: normalizePosition(fields.position),
        shot: shotResult.value, willTryout: parseWillTryout(fields.will_tryout),
        birthYear: parseInt(fields.birth_year) || null, dateOfBirth: parseDateOfBirth(fields.date_of_birth),
        gender: normalizeGender(fields.gender), externalId: fields.external_id || null,
        warnings: [], errors: [], assignedSession: null, status: 'ok',
      };
      if (!result.firstName)  result.errors.push('Missing first_name');
      if (!result.lastName)   result.errors.push('Missing last_name');
      if (!result.jerseyNumber || isNaN(result.jerseyNumber)) result.errors.push('Missing or invalid jersey_number');
      if (fields.shot && shotResult.warn) result.warnings.push(`Unrecognized shot value "${fields.shot}" — will be stored as NULL`);
      if (result.jerseyNumber && batchJerseyCount[result.jerseyNumber] > 1) result.warnings.push(`Jersey #${result.jerseyNumber} appears more than once in this file`);
    ...
    const summary = { total: preview.length, valid: preview.filter(r => r.status === 'ok' || r.status === 'update').length, warnings: preview.filter(r => r.warnings.length > 0).length, errors: preview.filter(r => r.status === 'error').length };
...
router.post('/commit', ...guard, async (req, res) => {
  console.warn('[deprecated] POST /api/import/commit — use /api/events/:eventId/import/:batchId/commit');
  const { csvText, eventId, ageGroupId } = req.body;
  if (!csvText || !eventId || !ageGroupId) {
    return res.status(400).json({ error: 'csvText, eventId, ageGroupId required' });
  }
  ...
  try {
    ...
    for (const row of rows) {
      ...
      try {
        const player = await findOrCreatePlayer(client, { firstName, lastName, dateOfBirth, gender, externalId, shot, birthYear });
        const registration = await upsertPlayerRegistration(client, { playerId: player.id, eventId, ageGroupId, jerseyNumber, position, shot, willTryout, outcome: null });
        if (registration.was_updated) {
          updated.push({ id: registration.id, player_id: player.id, first_name: player.first_name, last_name: player.last_name, jersey_number: registration.jersey_number, age_group_id: registration.age_group_id, event_id: registration.event_id, position: registration.position, shot: registration.shot, gender: player.gender, external_id: player.external_id });
        } else {
          added.push({ id: registration.id, player_id: player.id, first_name: player.first_name, last_name: player.last_name, jersey_number: registration.jersey_number, age_group_id: registration.age_group_id, event_id: registration.event_id, position: registration.position, shot: registration.shot, gender: player.gender, external_id: player.external_id });
        }
        await assignPlayerToSessions(client, player.id, ageGroupId, eventId);
      } catch (e) {
        errors.push({ firstName, lastName, jerseyNumber, reason: e.code === '23505' ? 'player or registration conflict' : e.message });
      }
    }
    await client.query('COMMIT');
    res.json({ summary: { added: added.length, updated: updated.length, errors: errors.length }, added, updated, errors });
  } catch (err) {
